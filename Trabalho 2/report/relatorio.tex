...
Esta operação tem de ser efetuada sequencialmente pois o pixel seguinte depende sempre do anterior, pelo que se recorreu à seguinte implementação para a função \textit{iterate\_direction\_dirxpos()} e para a função \textit{iterate\_direction\_dirxneg()}:
...
Foi então desenvolvido o seguinte código para as funções \textit{iterate\_direction\_dirypos()} e \textit{iterate\_direction\_diryneg()}:
